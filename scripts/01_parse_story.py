...
7. visual.prompt 质量要求：
   - 必须包含具体的环境描述（不能只写 "a room"，要写 "dimly lit concrete room with fluorescent lights flickering"）
   - 必须包含人物的动作/姿态（不能只写 "a man"，要写 "young man standing tensely with clenched fists, jaw tight"）
   - 必须包含光线描述（如 "harsh overhead light", "warm sunset glow through window", "cold moonlight casting long shadows"）
   - 多角色场景必须描述空间关系（如 "facing each other across a table", "standing behind with hand on shoulder"）
   - 禁止使用抽象词汇（如 "mysterious", "beautiful"），改用具体视觉描述
   - 每个 prompt 30-60 个英文单词，确保细节丰富
"""
...

{
  ...
  "characters_in_chapter": [
    {"name": "角色名", "appearance": "英文外貌描述，包含服装、发型、体型、显著特征，20词以上"}
  ],
  "plot_phases": [
